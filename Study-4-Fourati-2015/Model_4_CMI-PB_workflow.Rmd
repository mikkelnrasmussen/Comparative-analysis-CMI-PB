...
cell.types <- fread("../../Data/2021_cmipb_challenge/cell_types.txt")
...
# Cell frequnecy data
cytof.2020 <- fread("../../Data/2021_cmipb_challenge/2020LD_live_cell_percentages.csv")
cytof.2021 <- fread("../../Data/2021_cmipb_challenge/2021BD_live_cell_percentages.csv")
...
df.BioAge.2020.pred.TPM <- df.BioAge.2020.pred.TPM %>% 
   dplyr::mutate(titer.class = ifelse(titer.class == 1, 0, 1))

# Evaluate gene signature with TPM normalized gene expression
Fourati.2015.BioAge.2020.results.TPM <- evaluateModel(df.prediction=df.BioAge.2020.pred.TPM,
...
# Save results
save(Fourati.2015.BioAge.2020.results.TPM, 
     file = "Results/2020/TPM/BioAge/Fourati.2015.BioAge.2020.results.TPM.RData")
...
Fourati.2015.BioAge.2020.results.DESEQ <- evaluateModel(df.prediction=df.BioAge.2020.pred.DESEQ,
...
# Save results
save(Fourati.2015.BioAge.2020.results.DESEQ, 
     file = "Results/2020/DESEQ/BioAge/Fourati.2015.BioAge.2020.results.DESEQ.RData")
```
...
Fourati.2015.M1.2020.results.TPM <- evaluateModel(df.prediction=df.M1.2020.pred.TPM,
                                                  N.perm = 1000, direction = "missing",
                                                  add.to.title='- TPM normalized',
...
# Save results
save(Fourati.2015.M1.2020.results.TPM, 
     file = "Results/2020/TPM/M1/Fourati.2015.M1.2020.results.TPM.RData")
...
Fourati.2015.M1.M16.2020.results.TPM <- evaluateModel(df.prediction=df.M1.M16.2020.pred.TPM,
                                                      N.perm = 1000, direction = "missing",
                                                      add.to.title='- TPM normalized',
...
# Save results
save(Fourati.2015.M1.M16.2020.results.TPM, 
     file = "Results/2020/TPM/M1+M16/Fourati.2015.M1.M16.2020.results.TPM.RData")
...
Fourati.2015.M1.2020.results.DESEQ <- evaluateModel(df.prediction=df.M1.2020.pred.DESEQ,
                                                    N.perm = 1000, direction = "missing",
                                                    add.to.title='- DESeq2 VST normalized',
                                                    score.label = 'M1 BioAge score',
...
# Save results
save(Fourati.2015.M1.2020.results.DESEQ, 
     file = "Results/2020/DESEQ/M1/Fourati.2015.M1.2020.results.DESEQ.RData")
...
Fourati.2015.M1.M16.2020.results.DESEQ <- evaluateModel(df.prediction=df.M1.M16.2020.pred.DESEQ,
                                                        N.perm = 1000, direction = "missing",
                                                        add.to.title='- DESeq2 VST normalized',
                                                        score.label = 'M1+M16 BioAge score',
...
# Save results
save(Fourati.2015.M1.M16.2020.results.DESEQ, 
     file = "Results/2020/DESEQ/M1+M16/Fourati.2015.M1.M16.2020.results.DESEQ.RData")
...
load(file='../CMI-PB/em131.naiveBayes.RData')
NB.fit <- fit

################ TPM normalized gene expression ###############
# Calculate the gene signature score with DESeq2 normalized gene expression
df.NB.2020.pred.TPM <- mlModelPredict(gene.ex.data=ge.TPM.2020.day0.T,
                                      model=NB.fit, study='fourati_2015_NB',
                                      post.ab.data=abt.2020.day14, 
                                      pre.ab.data=abt.2020.day0,
                                      filter.ab.ag=selected.iso.ag, 
                                      verbose=TRUE)
...
# Evaluate gene signature with TPM normalized gene expression
Fourati.2015.NB.2020.results.TPM <- evaluateModel(df.prediction=df.NB.2020.pred.TPM,
                                                  N.perm = 1000, direction = "missing",
                                                  add.to.title='- TPM normalized',
...
# Save results
save(Fourati.2015.NB.2020.results.TPM, 
     file = "Results/2020/TPM/Naive_bayes/Fourati.2015.NB.2020.results.TPM.RData")
...
# Evaluate gene signature with DESeq2 normalized gene expression
Fourati.2015.NB.2020.results.DESEQ <- evaluateModel(df.prediction=df.NB.2020.pred.DESEQ,
                                                    N.perm = 1000, direction = "missing",
                                                    add.to.title='- DESeq2 VST normalized',
                                                    score.label = 'Ratio between post-probalities',
                                                    fn1='NBpred_vs_day14_heatmap',
...
# Save results
save(Fourati.2015.NB.2020.results.DESEQ, 
     file = "Results/2020/DESEQ/Naive_bayes/Fourati.2015.NB.2020.results.DESEQ.RData")
...
load(file='../CMI-PB/em131.logisticReg.RData')
LR.fit <- fit
...
Fourati.2015.LR.2020.results <- evaluateModel(df.prediction=df.LR.2020.pred, 
...
# Save results
save(Fourati.2015.LR.2020.results, 
     file = "Results/2020/LR/Fourati.2015.LR.2020.results.RData")
...
# BioAge predictions
Fourati.2015.BioAge.2021.pred <- submission.format %>% 
   left_join(., df.BioAge.pred.2021, by="subject_id")
Fourati.2015.BioAge.2021.pred[, c(5:11)] <- Fourati.2015.BioAge.2021.pred$rank.predictor
Fourati.2015.BioAge.2021.pred <- Fourati.2015.BioAge.2021.pred[,c(1:18)] %>% 
   dplyr::rename(Subject_ID = subject_id)
# Save predictions
write.table(Fourati.2015.BioAge.2021.pred, 
            file = "Results/2021_predictions/fourati_2015_BioAge_CMI-PB_2021.tsv",
            quote=FALSE, sep='\t', row.names = FALSE)

# M1 predictions
Fourati.2015.M1.2021.pred <- submission.format %>% 
   left_join(., df.M1.pred.2021, by="subject_id")
Fourati.2015.M1.2021.pred[, c(5:11)] <- Fourati.2015.M1.2021.pred$rank.predictor
Fourati.2015.M1.2021.pred <- Fourati.2015.M1.2021.pred[,c(1:18)] %>% 
   dplyr::rename(Subject_ID = subject_id)
# Save predictions
write.table(Fourati.2015.M1.2021.pred, 
            file = "Results/2021_predictions/fourati_2015_M1_CMI-PB_2021.tsv",
            quote=FALSE, sep='\t', row.names = FALSE)

# M1+M16 predictions
Fourati.2015.M1.M16.2021.pred <- submission.format %>% 
   left_join(., df.M1.M16.pred.2021, by="subject_id")
Fourati.2015.M1.M16.2021.pred[, c(5:11)] <- Fourati.2015.M1.M16.2021.pred$rank.predictor
Fourati.2015.M1.M16.2021.pred <- Fourati.2015.M1.M16.2021.pred[,c(1:18)] %>% 
   dplyr::rename(Subject_ID = subject_id)
# Save predictions
write.table(Fourati.2015.M1.M16.2021.pred, 
            file = "Results/2021_predictions/fourati_2015_M1+M16_CMI-PB_2021.tsv",
            quote=FALSE, sep='\t', row.names = FALSE)


# Naive Bayes predictions
Fourati.2015.NB.2021.pred <- submission.format %>% 
   left_join(., df.NB.pred.2021, by="subject_id")
Fourati.2015.NB.2021.pred[, c(5:11)] <- Fourati.2015.NB.2021.pred$rank.predictor
Fourati.2015.NB.2021.pred <- Fourati.2015.NB.2021.pred[,c(1:18)] %>% 
   dplyr::rename(Subject_ID = subject_id)
# Save predictions
write.table(Fourati.2015.NB.2021.pred, 
            file = "Results/2021_predictions/fourati_2015_NB_CMI-PB_2021.tsv",
            quote=FALSE, sep='\t', row.names = FALSE)

# Logistic regression predictions
Fourati.2015.LR.2021.pred <- submission.format %>% 
   left_join(., df.LR.pred.2021, by="subject_id")
Fourati.2015.LR.2021.pred[, c(5:11)] <- Fourati.2015.LR.2021.pred$rank.predictor
Fourati.2015.LR.2021.pred <- Fourati.2015.LR.2021.pred[,c(1:18)] %>% 
   dplyr::rename(Subject_ID = subject_id)
# Save predictions
write.table(Fourati.2015.LR.2021.pred, 
            file = "Results/2021_predictions/fourati_2015_LR_CMI-PB_2021.tsv",
            quote=FALSE, sep='\t', row.names = FALSE)
...
# Evaluate gene signature with DESeq2 normalized gene expression
Fourati.2015.BioAge.2021.results <- evaluateModel(df.prediction=df.BioAge.2021.pred,
                                                  N.perm = 1000, direction = "missing",
                                                  add.to.title='- DESeq2 VST normalized',
                                                  score.label = 'BioAge score',
...
# Save results
save(Fourati.2015.BioAge.2021.results, 
     file = "Results/2021/BioAge/Fourati.2015.BioAge.2021.results.RData")
...
# Evaluate M1 BioAge score with DESeq2 normalized gene expression
Fourati.2015.M1.2021.results <- evaluateModel(df.prediction=df.M1.2021.pred,
                                              N.perm = 1000, direction = "missing",
                                              add.to.title='- DESeq2 VST normalized',
                                              score.label = 'M1 BioAge score',
...
# Save results
save(Fourati.2015.M1.2021.results, 
     file = "Results/2021/M1/Fourati.2015.M1.2021.results.RData")
...
# Evaluate M1+M16 BioAge score with DESeq2 normalized gene expression
Fourati.2015.M1.M16.2021.results <- evaluateModel(df.prediction=df.M1.M16.2021.pred,
                                                  N.perm = 1000, direction = "missing",
                                                  add.to.title='- DESeq2 VST normalized',
                                                  score.label = 'M1+M16 BioAge score',
...
# Save results
save(Fourati.2015.M1.M16.2021.results, 
     file = "Results/2021/M1+M16/Fourati.2015.M1.M16.2021.results.RData")

# Calculate the gene signature score with DESeq2 normalized gene expression
df.NB.2021.pred <- mlModelPredict(gene.ex.data=ge.norm.2021.day0,
                                  model=NB.fit, 
                                  study='fourati_2015_NB',
                                  post.ab.data=abt.2021.day14,
                                  pre.ab.data=abt.2021.day0,
                                  filter.ab.ag=selected.iso.ag,
                                  verbose=TRUE)
...
# Evaluate gene signature with DESeq2 normalized gene expression
Fourati.2015.NB.2021.results <- evaluateModel(df.prediction=df.NB.2021.pred,
                                              N.perm = 1000, direction = "missing",
                                              add.to.title='- DESeq2 VST normalized',
                                              score.label = 'Ratio between post-probalities',
                                              fn1='NBpred_vs_day14_heatmap',
...
# Save results
save(Fourati.2015.NB.2021.results, 
     file = "Results/2021/Naive_bayes/Fourati.2015.NB.2021.results.RData")
...
# Evaluate the logistic regression model with 4 cell populations
Fourati.2015.LR.2021.results <- evaluateModel(df.prediction=df.LR.2021.pred, 
                                              direction = 'missing', N.perm = 1000,
                                              score.label='LR output - scale of the linear predictors',
                                              add.to.title = " - Logistic regression using cell populations",
                                              fn1='LRpred_vs_day14_heatmap',
...
# Save results
save(Fourati.2015.LR.2021.results, 
     file = "Results/2021/LR/Fourati.2015.LR.2021.results.RData")
